...
SAMSARA_DRIVER_COACH_ASSIGNMENTS_API_URL = (
    "https://api.samsara.com/coaching/driver-coach-assignments"
)
...
def driver_safety_scores(context, driverId):
    ...
    SCORE_PERIODS = [
        (
            int((TIME_NOW - datetime.timedelta(hours=24)).timestamp() * 1000),
            int(TIME_NOW.timestamp() * 1000),
        ),
        (
            int(
                (TIME_NOW - datetime.timedelta(days=SCORE_PERIOD_SIZE_DAYS)).timestamp()
                * 1000
            ),
            int(
                (
                    TIME_NOW - datetime.timedelta(days=SCORE_PERIOD_SIZE_DAYS - 1)
                ).timestamp()
                * 1000
            ),
        ),
    ...
    for startMs, endMs in SCORE_PERIODS:
        URL = SAMSARA_DRIVER_SAFETY_SCORE_API_URL.format(
            driverId=driverId, startMs=startMs, endMs=endMs
        )
        response = requests.get(URL, headers=SAMSARA_API_HEADERS)

        if response.status_code != 200:
            raise Exception(
                f"Failed to get driver safety score: {response.status_code} {response.text}"
            )
...
def driver_coach_assignments(context):
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to get driver coach assignments: {response.status_code} {response.text}"
        )
...
def name_of_coach(context, coachId):
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to get coach name: {response.status_code} {response.text}"
        )
...
def main(event, context):
    ...
    for driverId, coachId in coach_assignments:
        ...
        print(
            f"Driver {driverId} has coach {coachId} and scores: {safety_scores['previous_score']} -> {safety_scores['current_score']}"
        )
        ...
        if coachId not in coaches:
            coaches[coachId] = {
                "score": 0,
                "total_drivers": 0,
                "drivers_with_score_reduction": 0,
                "drivers_with_score_increase": 0,
            }
    ...
    for coachId, coach in coaches.items():
        if (
            coach["drivers_with_score_reduction"] / coach["total_drivers"]
            >= SCORE_REDUCTION_THRESHOLD
        ):
            coach["score"] = -1
        elif (
            coach["drivers_with_score_increase"] / coach["total_drivers"]
            >= SCORE_INCREASE_THRESHOLD
            and coach["drivers_with_score_reduction"] == 0
        ):
            coach["score"] = 1

        coach_name = name_of_coach(context, coachId)
        score_display = (
            "+1" if coach["score"] > 0 else "-1" if coach["score"] < 0 else "0"
        )
